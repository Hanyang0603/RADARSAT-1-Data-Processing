...
first_rg_line = 8 * ( ceil(first_rg_line / 8) - 1 ) + 1;%每8个为一组，以每组的第一个为起始方位采样

% Make 'Nrg_lines_blk' a multiple of 8, to get complete the 8-line blocks
Nrg_lines_blk = 8 * ceil(Nrg_lines_blk / 8);%每一个block含有的line数，得到一个8-line的block 

% Find the number of complete blocks required to cover the area of interest
Nblocks = ceil(Nrg_lines / Nrg_lines_blk);% 一共需要的block数量，一般是2

% Make 'Nrg_lines' a multiple of 'Nblocks', to get complete blocks
Nrg_lines = Nrg_lines_blk * Nblocks; %方位采样数量
