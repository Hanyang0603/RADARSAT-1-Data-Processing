...
input_path        = 'D:\My Thesis\Thesis II\Programs\Radarsat-1 Real Data Experiment\';
output_path       = 'D:\My Thesis\Thesis II\Programs\Radarsat-1 Real Data Experiment\';
...
first_rg_cell   =  760;    % Define the range limits: starting range sample
first_rg_line   =  7193 + 128;    % Define the azimuth limits (19432 max): starting azimuth sample, must be a multiple of 8 plus 1

Nrg_cells       =  2048;    % Suggest 2048 cells: number of range samples
Nrg_lines_blk   =  1536;    % Suggest 1536, it should be larger than the size of the azimuth match filter (705)
                            % Number of azimuth samples, must be a multiple of 8
                            % Every 8 azimuth lines are stored together, so reading in multiples of 8 is convenient

Nrg_lines       =  1 * Nrg_lines_blk;  % Total number of azimuth samples, set to 2 here, basically extracting two file blocks
